...
bool AppSettings::Load()
{
    ...
    if (fs::exists(filepath.c_str())) {
        if (m_doc.LoadFile((filepath.c_str())) != tx::XML_SUCCESS) {
            std:cerr << "ERROR while loading [" << filepath << "]" << std::endl;
            return false;
        }
        tx::XMLHandle hDoc(&m_doc);
        tx::XMLElement* pElem;
...
//		for (auto& iter=m_user_info_map.begin(); iter != m_user_info_map.end(); iter++)
		for (auto& iter : m_map_user_info)
		{
			...
			usr_info = doc.NewElement(key.c_str());
			usr_info->LinkEndChild( doc.NewText(value.c_str()));
			usr_info->LinkEndChild( usr_info );
		}
	}
	{
